...
if nargin < 2 || isempty(data_dir)
    data_dir = fullfile('/Users/erik/Library/CloudStorage/Dropbox/Work/UFO/Data', patient, 'UFO_edited');
end
...
try
    ...
    % Verify each cell contains Nx3 matrix
    for ii = 1:numel(UFOdat)
        if ~isempty(UFOdat{ii})
            if size(UFOdat{ii}, 2) ~= 3
                error('UFOdat_edited{%d} must have 3 columns [start_us, end_us, freq_hz]', ii);
            end
        end
    end
